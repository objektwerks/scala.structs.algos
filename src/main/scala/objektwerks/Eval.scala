...
object Eval:
  ...
  def eval(expr: String): Int =
    ...
    def evalSimpleOperation(op1: Int, op2: Int, operator: String) =
      operator match
        case "+" => op1 + op2
        case "-" => op1 - op2
        case "*" => op1 * op2
    ...
    def prioritizeOperator(operator: String): Int =
      operator match
        case "+" | "-" => 1
        case "*" | "/" => 2
        case _ => 0
    ...
    def loop(remainingOperands: List[Int],
      ...
      if remainingOperands.isEmpty then
        if operatorStack.isEmpty then operandStack.head // final result
        else
      ...
      else if operatorStack.isEmpty || prioritizeOperator(operatorStack.head)
              < prioritizeOperator(remainingOperators.head) then
